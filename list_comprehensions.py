...
johns_nested_method = [fruit for fruit in fruits if len([letter for letter in fruit if letter.lower() in 'aeiou']) > 2]
johns_nested_method
...
johns_two_step_method = [fruit for fruit in fruits if has_only_two_vowels(fruit) == True]
johns_two_step_method
...
number_of_characters_per_fruit
...
# prime = num > 1, only divisible by 1 and itself: num % i != 0 where i > 1 and i != num